...
function DsrExcelExport({ month, flatData, filteredData, paginatedData }) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const optionText = exportFiltered ? 'filtered' : (exportPaginated ? 'page' : 'all')
      link.download = `DSR_${month}_${optionText}_${dateStr}.csv`
  ...
  const getDataCount = () => {
    if (exportFiltered) return filteredData.length
    if (exportPaginated) return paginatedData.length
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      ...
      <button
        ...
        className={`
          ...
          ${loadingExport 
            ...
            : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 active:scale-95 text-white'
          }
        `}
      >
        {loadingExport ? (
      ...
      {/* Dropdown Menu */}
      {dropdownOpen && (
        <div className="absolute top-full left-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden animate-slideDown">
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-3 border-b border-gray-200">
            <h3 className="font-semibold text-gray-800 text-sm">Export DSR Data</h3>
          ...
          <div className="p-3 space-y-3">
            <label className="flex items-start gap-3 cursor-pointer group">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.checked) setExportPaginated(false)
                }}
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              <input
                ...
                onChange={(e) => {
                  setExportPaginated(e.target.checked)
          ...
          {(exportFiltered || exportPaginated) && (
            <div className="mx-3 mb-3 px-3 py-2 bg-blue-50 rounded-lg border border-blue-100">
              <div className="flex items-center gap-2 text-xs text-blue-700">
                ...
                <span>
                  Exporting: {exportFiltered ? 'Filtered Data' : 'Current Page'} 
                  {' '}({getDataCount()} rows)
                </span>
              </div>
            </div>
          ...
          {/* Total Rows Info */}
          {!exportFiltered && !exportPaginated && (
            <div className="mx-3 mb-3 px-3 py-2 bg-gray-50 rounded-lg border border-gray-100">
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-600">Total available rows:</span>
                <span className="font-semibold text-gray-800">{flatData.length}</span>
              </div>
            </div>
          )}
          ...
          <div className="border-t border-gray-100 p-3 flex gap-2">
            ...
            <button
              ...
              className="flex-1 px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl hover:from-blue-700 hover:to-indigo-700 active:scale-95 transition-all duration-200 shadow-sm"
            >
              Export Now
            </button>
          </div>
